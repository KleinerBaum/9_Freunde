...
if 'user' not in st.session_state or st.session_state.get('user') is None:
    ...
    if user_role == 'admin':
        menu = st.sidebar.radio("Navigationsmenü", ("Stammdaten", "Dokumente", "Fotos", "Kalender"), index=0)
    else:
        menu = st.sidebar.radio("Menü", ("Mein Kind", "Dokumente", "Fotos", "Termine"), index=0)
    ...
    if user_role == 'admin':
        ...
        if menu == "Stammdaten":
            ...
            if children:
                for child in children:
                    st.write(f"- **{child.get('name')}** (Eltern: {child.get('parent_email')})")
            else:
            ...
            st.info("Beim Anlegen eines Kindes wird automatisch ein zugehöriger Drive-Ordner erstellt und verknüpft.")
        ...
        # ---- Admin: Dokumente ----
            ...
            if not children:
                ...
                selected_name = st.selectbox("Für welches Kind soll ein Dokument erstellt werden?", child_names)
                # find selected child data
                sel_child = next(child for child in children if child.get('name') == selected_name)
                doc_notes = st.text_area("Stichpunkte oder Notizen für das Dokument:")
                save_to_drive = st.checkbox("Dokument im Drive-Ordner des Kindes speichern?")
                if st.button("Dokument erstellen"):
                    with st.spinner("Generiere Dokument mit OpenAI..."):
                        try:
                            doc_bytes, file_name = doc_agent.generate_document(sel_child, doc_notes)
                            st.success("Dokument erstellt: " + file_name)
                            # Download-Button anzeigen
                            st.download_button("📄 Dokument herunterladen", data=doc_bytes, file_name=file_name)
                            # Optional: in Drive speichern
                            if save_to_drive:
                                folder_id = sel_child.get('folder_id')
                                if folder_id:
                                    drive_agent.upload_file(file_name, doc_bytes, "application/vnd.openxmlformats-officedocument.wordprocessingml.document", folder_id)
                                    st.info("Dokument wurde im Drive-Ordner gespeichert.")
                                else:
                                    st.error("Kein Drive-Ordner für dieses Kind vorhanden.")
                        except Exception as e:
                            st.error(f"Fehler bei der Dokumentenerstellung: {e}")

                # Optionale Liste vorhandener Dokumente im Drive
                if sel_child.get('folder_id'):
                    docs_list = drive_agent.list_files(sel_child['folder_id'], mime_type_filter='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
                    if docs_list:
                        ...
                        for doc in docs_list:
                            ...
                            st.download_button("Download", data=drive_agent.download_file(file_id), file_name=file_name, key=file_id)
                    else:
        ...
        # ---- Admin: Fotos ----
            ...
            if not children:
                ...
                sel_child = next(child for child in children if child.get('name') == selected_name)
                image_file = st.file_uploader("Foto auswählen", type=['jpg','jpeg','png'])
                if image_file and st.button("Upload Foto"):
                    try:
                        # Upload via PhotoAgent (speichert in Drive)
                        photo_agent.upload_photo(image_file, sel_child.get('id') if sel_child else None, sel_child.get('folder_id'))
                        st.success(f"Foto für {selected_name} hochgeladen.")
                        st.image(image_file, caption=f"Hochgeladenes Foto: {image_file.name}", use_column_width=True)
                    except Exception as e:
                        st.error(f"Fehler beim Foto-Upload: {e}")
                st.info("Fotos werden sicher auf Google Drive gespeichert und nur den berechtigten Eltern angezeigt.")
        ...
        # ---- Admin: Kalender ----
            ...
            if st.button("Termin hinzufügen"):
                if not title:
                    ...
                    try:
                        cal_agent.add_event(title, date, time, description, all_day=all_day)
        ...
        if menu == "Mein Kind":
            ...
            if child and child.get('folder_id'):
                docs_list = drive_agent.list_files(child['folder_id'], mime_type_filter='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
                if docs_list:
                    for doc in docs_list:
                        file_name = doc.get('name')
                        file_id = doc.get('id')
                        st.markdown(f"**{file_name}** ")
                        st.download_button("Herunterladen", data=drive_agent.download_file(file_id), file_name=file_name, key=file_id)
                else:
            ...
            st.subheader("Fotos")
            if child and child.get('folder_id'):
                photos = drive_agent.list_files(child['folder_id'], mime_type_filter='image/')
                if photos:
                    for photo in photos:
                        img_bytes = drive_agent.download_file(photo['id'])
                        st.image(img_bytes, caption=photo.get('name'), use_column_width=True)
                else:
